...
Post offices have different inner structure. Specifically, each of them has some limitations on the packages it can store - their weight should be between min_weight and max-weight inclusively, where min_weight and max-weight are fixed for each office.
...
void print_all_packages(town t) //Function to print all packages
{
    printf("%s:\n", t.name);
    for (int i = 0; i < t.offices_count; i++) //Visit all the town offices using the for loop
    {
        printf("\t%d:\n", i);
        for (int j = 0; j < t.offices[i].packages_count; j++) //Visit every package of the currently indexed town office
        {
            printf("\t\t%s\n", t.offices[i].packages[j].id); //Prints the package and marks the end of the function
        }
    }
}
...
void send_all_acceptable_packages(town *source, int source_office_index, town *target, int target_office_index)
{
    ...
    int min = target->offices[target_office_index].min_weight; //Indicates the target pointer to the minimum value
    int max = target->offices[target_office_index].max_weight; //Indicates the target pointer to the maximum value
...
{
    int index = 0, max = 0; //Sets the index count and max value of packages to NULL
    for (int i = 0; i < towns_count; i++)   //Visit every town
    {
        int sum = 0;
        for (int j = 0; j < towns[i].offices_count; j++)
        {
            sum += towns[i].offices[j].packages_count;  //Continue adding the number of packages
        }
        if (sum > max)
        {
            max = sum;  //Set the sum as the maximum number of packages if it exceeds the previously set max value of packages
            index = i;
        }
    }
    return towns[index];
